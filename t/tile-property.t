...
is( $tile->get_rent, 2*$rent->[0], 'After purchase: since monopoly (only one tile in group), rent is doubled' );
...
# mortgaging
$tile->mortgage;
is( $player->money, ($money+=$price/2+$houses_cost), 'After mortgage (with houses): player cash is correct' );
